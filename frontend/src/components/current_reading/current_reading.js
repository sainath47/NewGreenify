...
return `${month}/${day}/${year}`;
  ...
  const eD = formatDate(endDate)
      ...
      const response = await fetch(`http://localhost:8000/api/reading?startDate=${sD}&endDate=${eD}&houseNo=${houseNo}&blockNo=${blockNo}&meterSNo=${meterSNo}
      `);
      const data = await response.json();
...
<ExportFile readings={readings} />
     ...
     {
         !readings && <div className="flex align-middle justify-center">No Data Corresponding</div>
     }
